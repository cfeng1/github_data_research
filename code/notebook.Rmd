...
df_temp <- df %>%
           ...
           summarise(total_commits=sum(n_commits),months_active=max(months_active),n_authors=max(n_authors))

store <- matrix(0,6,6)           
as.numeric(df_temp %>% filter(months_active<=1) %>% summarise(n_projects = n(),mean(total_commits)))
...
store[1,] <- store_fun(df_temp,0,1)
store[2,] <- store_fun(df_temp,2,3)
store[3,] <- store_fun(df_temp,4,6)
store[4,] <- store_fun(df_temp,7,12)
store[5,] <- store_fun(df_temp,13,36)
store[6,] <- store_fun(df_temp,37,1000)

colnames(store) <- c('N. Projects','Mean Total Commits','Max Total Commits','Median Total Commits','Mean N. Authors')
