...
df_temp <- df %>%
           ...
           filter(total_commits>=filter_productivity_commits_min,total_commits<filter_productivity_commits_max) %>%
           filter(n_commits>0) %>%
           summarize(period_active=n(),total_commits=sum(n_commits),n_projects=max(n_projects),n_languages=max(n_languages)) 
...
store <- matrix(0,6,7)
store[1,1:5] <- compute_table(df_temp,0,1)
store[2,1:5] <- compute_table(df_temp,2,3)
store[3,1:5] <- compute_table(df_temp,4,6)
store[4,1:5] <- compute_table(df_temp,7,12)
store[5,1:5] <- compute_table(df_temp,13,36)
store[6,1:5] <- compute_table(df_temp,37,200)                        
store[,6] <- store[,3] / sum(store[,3])
store[,7] <- store[,2] / sum(store[,2])
colnames(store) <- c('Average Tot. Commits', 'N. Users','Total Number of Commits','Average N. Of Projects','Avg. Number of Languages','Share of Total Commits','Share of Total Users')
rownames(store) <- c('1 month','2-3 months', '4-6 months', '7-12 months', '13-36 months', '36+ months')

print(round(store,3))
```

TO DO: Compute the number of different projects per month for each category of users, conditional on making some commits
...
plot_life_profile(7,12,'Probability of Making at least one Commit vs. Age, Active 6-12 months')
plot_life_profile(3,6,'Probability of Making at least one Commit vs. Age, Active 6-12 months')
...
TODO: create measure of people exit/death: 12 months after last commit/24 months + do some sensitivity analysis. Check how many come back after these periods of time 
