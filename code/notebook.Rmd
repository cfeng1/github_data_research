...
df_p <- df_commits %>%
        group_by(project_id) %>%
        summarize(project_created_at=min(commit_time),last_commit=max(commit_time),total_commits=n())
...
df_temp <- df %>%
           group_by(project_id) %>%
           filter(last_commit<='2018-06-01') %>%
           summarise(total_commits=sum(n_commits),months_active=max(months_active),)

           

as.numeric(df_temp %>% filter(total_commits<=10,total_commits>0) %>% summarise(n_projects = n(),mean(months_active)))

as.numeric(df_temp %>% filter(total_commits<=100,total_commits>10) %>% summarise(n_projects = n(),mean(months_active)))

as.numeric(df_temp %>% filter(total_commits<=1000,total_commits>100) %>% summarise(n_projects = n(),mean(months_active)))

as.numeric(df_temp %>% filter(total_commits<=10000,total_commits>1000) %>% summarise(n_projects = n(),mean(months_active)))

as.numeric(df_temp %>% filter(total_commits<=1000000,total_commits>10000) %>% summarise(n_projects = n(),mean(months_active)))
```


```{r}
min_total_commits <- 2500
max_total_commits <- 200000

df_temp <- df %>% 
           group_by(project_id) %>% 
           filter(sum(n_commits)>=min_total_commits,sum(n_commits)<=max_total_commits)

df_temp <- df_temp %>%
           group_by(project_age) %>%
           summarise(n=mean(n_commits),p=mean(as.numeric(n_commits>0)),p10=mean(as.numeric(n_commits>10)))
...
plot(df_temp$p[2:84],
     xlab='Age of the Projects in Months',
     ylab='N. Commits',
     main='Probability Project gets at least 1 commit vs. Age of Project')

plot(df_temp$p10[2:84],
     xlab='Age of the Projects in Months',
     ylab='N. Commits',
     main='Probability Project gets at least 10 commit vs. Age of Project')
```
...
df_p <- df_commits %>%
        group_by(author_id) %>%
        summarize(author_first_commit=min(commit_time),n_projects=n_distinct(project_id),author_last_commit=max(commit_time))
...
df <- df %>% 
      ...
      #filter(author_last_commit<='2018-06-01')
...
df_temp <- df %>%
           ...
           filter(total_commits>=filter_productivity_commits_min,total_commits<filter_productivity_commits_max) %>%
           filter(n_commits>0) %>%
           summarize(period_active=n(),total_commits=sum(n_commits),n_projects=max(n_projects)) 


store <- matrix(0,6,6)
store[1,1:4] <- as.numeric(df_temp %>% filter(period_active<=1) %>% summarise(average_total_commits=mean(total_commits),n_obs=n(),total_commits=sum(total_commits),average_projects=mean(n_projects)))
                        
store[2,1:4] <- as.numeric(df_temp %>% filter(period_active>1,period_active<=3) %>% summarise(average_total_commits=mean(total_commits),n_obs=n(),total_commits=sum(total_commits),average_projects=mean(n_projects)))

store[3,1:4] <- as.numeric(df_temp %>% filter(period_active>3,period_active<=6) %>% summarise(average_total_commits=mean(total_commits),n_obs=n(),total_commits=sum(total_commits),average_projects=mean(n_projects)))

                        
store[4,1:4] <- as.numeric(df_temp %>% filter(period_active>6,period_active<=12) %>% summarise(average_total_commits=mean(total_commits),n_obs=n(),total_commits=sum(total_commits),average_projects=mean(n_projects)))
                        
store[5,1:4] <- as.numeric(df_temp %>% filter(period_active>12,period_active<=36) %>% summarise(average_total_commits=mean(total_commits),n_obs=n(),total_commits=sum(total_commits),average_projects=mean(n_projects)))
                        
store[6,1:4] <- as.numeric(df_temp %>% filter(period_active>36) %>% summarise(average_total_commits=mean(total_commits),n_obs=n(),total_commits=sum(total_commits),average_projects=mean(n_projects)))

store[,5] <- store[,3] / sum(store[,3])
store[,6] <- store[,2] / sum(store[,2])
colnames(store) <- c('Average Tot. Commits', 'N. Users','Total Number of Commits','Average N. Of Projects','Share of Total Commits','Share of Total Users')
rownames(store) <- c('1 month','2-3 months', '4-6 months', '7-12 months', '13-36 months', '36+ months')

print(round(store,3))
```

Here try to understand if there is any observable pattern in the life-cycle of users. 
...
plot_life_profile <-function(lb,ub,title){
    ...
    legend("bottomleft", legend = c("Removed Users who Exit", "Included Users who Exit"),
...
plot_life_profile(37,200,'Probability of Making at least one Commit vs. Age, Active 36+')
plot_life_profile(13,36,'Probability of Making at least one Commit vs. Age, Active 13-36')
plot_life_profile(6,12,'Probability of Making at least one Commit vs. Age, Active 6-12')
